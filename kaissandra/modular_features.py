...
def compress_zip(config, sources, features=[i for i in range(37)], 
    ...
    for assetsdir in tqdm(assetsdirs, mininterval=1):#os.walk(path)
        try:
            ...
            print("\nWARNING! File "+rootdir+assetsdir+" could not be found. Skipped")
            continue
        for file in list_all_dirs:
            if os.path.isdir(rootdir+assetsdir+file):
...
if __name__=='__main__':
    ...
    build_test_db = False
    save_stats = True
    feature_keys = [i for i in range(2000,2014)]+[i for i in range(2015,2019)]##[i for i in range(37)]#[i for i in range(37,48)]+
    config_name = 'CFEATS1000'
