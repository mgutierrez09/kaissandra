...
def loadTR(IDresults,resultsDir,saveResults,startFrom):
    
    # Load results
    if saveResults:
        if os.path.exists(resultsDir+"TR.p"):
            TR = pickle.load( open( resultsDir+"TR.p", "rb" ))
            if IDresults in TR:
                if startFrom<0:
                    lastSaved = TR[IDresults]
                    
                else:
                    lastSaved = startFrom
                print("Results table found and loaded")
            else:
                if startFrom<0:
                    lastSaved = 0
                else:
...
def getResults(Y_tilde, Y_real, softMax, DTA, size_output_layer):
    ...
    expectations = Y_tilde[nonZerosYt]
    outcomes = Y_real[nonZerosYt]
    accDirectionAllInd = np.sign(expectations)-np.sign(outcomes)
    ...
    for l in range(int((size_output_layer-1)/2)):
        levelL = np.abs(Y_tilde)==l+1
        lYt_tr = Y_tilde[levelL*nonZerosYtYr]
        lYt_t = Y_tilde[levelL*nonZerosYt]
        lYr_tr = Y_real[nonZerosYtYr*levelL]
        lYr_t = Y_real[nonZerosYt*levelL]
        lNZ = lYt_tr.shape[0] # NZ in range r
        lNZA = lYt_t.shape[0]
        accLevelInd = np.sign(lYt_tr)-np.sign(lYr_tr)
        accLevelAllInd = np.sign(lYt_t)-np.sign(lYr_t)
        lRD = np.sum(accLevelInd==0)
        lRDA = np.sum(accLevelAllInd==0)
        #rJournal = getJournal(DTA.iloc[percent*nonZerosYt], rYt_t, rYr_t, np.sign(rYt_t)-np.sign(rYr_t))
        #print(rJournal)
        results["level"+str(l+1)] = [lRD, lNZ, lRDA, lNZA]
        
    #print(softMax.shape)
    probLong = np.sum(softMax[:,int((size_output_layer-1)/2)+1:],axis=1)
    probShort = np.sum(softMax[:,:int((size_output_layer-1)/2)],axis=1)
    probLoOrSho = np.maximum(probLong,probShort)
    percents = np.array([1, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3])
    
    for per in range(percents.shape[0]-1):
        percent = (probLoOrSho<percents[per]) * (probLoOrSho>=percents[per+1])
        rYt_tr = Y_tilde[percent*nonZerosYtYr]
        rYt_t = Y_tilde[percent*nonZerosYt]
        rYr_tr = Y_real[nonZerosYtYr*percent]
        rYr_t = Y_real[nonZerosYt*percent]
        rNZ = rYt_tr.shape[0] # NZ in range r
        rNZA = rYt_t.shape[0]
        accRangeInd = np.sign(rYt_tr)-np.sign(rYr_tr)
        accRangeAllInd = np.sign(rYt_t)-np.sign(rYr_t)
        rRD = np.sum(accRangeInd==0)
        rRDA = np.sum(accRangeAllInd==0)
        #rJournal = getJournal(DTA.iloc[percent*nonZerosYt], rYt_t, rYr_t, np.sign(rYt_t)-np.sign(rYr_t))
        #print(rJournal)
        results["percent"+str(int(100*percents[per+1]))] = [rRD, rNZ, rRDA, rNZA]
    

    Journal = pd.DataFrame()
    if 'A1' in DTA.columns:
        Journal,rGOIxLevel,rROIxPercent = getJournal(DTA.iloc[nonZerosYt], expectations, outcomes,
                             accDirectionAllInd,probLoOrSho[nonZerosYt],size_output_layer)
        tROI = Journal["ROI"].loc["Total"]
        tGROI = Journal["GROI"].loc["Total"]
        tSpread = Journal["Spread"].loc["Total"]
        rROI = Journal["ROI"].loc["Real"]
        rGROI = Journal["GROI"].loc["Real"]
        rSpread = Journal["Spread"].loc["Real"]
        
    # save results
    results["accuracy"] = 100*accuracy
    results["accDirection"] = 100*accDirection
    results["accDirectionAll"] = 100*accDirectionAll
    results["percentNonZeros"] = 100*percentNonZeros
    results["percentNonZerosAll"] = 100*percentNonZerosAll
    results["NZ"] = NZ
    results["RD"] = RD
    results["NZA"] = NZA
    results["RDA"] = RDA
    ...
    if 'A1' in DTA.columns:
        results["tROI"] = tROI
        results["tGROI"] = tGROI
        results["tSpread"] = tSpread
        results["rROI"] = rROI
        results["rGROI"] = rGROI
        results["rSpread"] = rSpread
        results["rROIxLevel"] = rGOIxLevel
        results["rROIxPercent"] = rROIxPercent
    else:
        results["tROI"] = 0
        results["tGROI"] = 0
        results["tSpread"] = 0
        ...
        results["rGROI"] = 0
        results["rSpread"] = 0
        results["rROIxLevel"] = np.zeros((int((size_output_layer-1)/2)))
        results["rROIxPercent"] = np.zeros((percents.shape[0]-1))
        results["ROIxLevel"] = np.zeros((int((size_output_layer-1)/2)))
        results["ROIxPercent"] = np.zeros((percents.shape[0]-1))

    return results

def getJournal(infoBets, expectations, outcomes, accDirectionAllInd, prob, size_output_layer):
    
    Journal = infoBets.drop("A1",axis=1).drop("A2",axis=1).drop("B2",axis=1
                           ).drop("B1",axis=1)
    #print(infoBets)
    Journal.columns = ['Entry Time         ','Exit Time          ','Asset']
    grossROIs = np.sign(expectations)*(infoBets["B2"]-infoBets["B1"])/infoBets["B1"]
    spreads =  (infoBets["A2"]-infoBets["B2"])/infoBets["B1"]
    # include stoploss in strategy
    stoploss = 2*spreads
    #print(stoploss)
    ...
    Journal.loc[:,'Bet'] = expectations.astype(int)
    Journal.loc[:,'Outcome'] = outcomes.astype(int)
    Journal.loc[:,'Diff'] = accDirectionAllInd.astype(int)
    Journal.loc[:,"Prob"] = prob.astype(float)
    ...
    eInit = 0
    percents = np.array([1, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3])
    rROIxPercent = np.zeros((percents.shape[0])-1)
    rGOIxLevel = np.zeros((int((size_output_layer-1)/2)))
    
    
    for e in range(1,Journal.shape[0]):

        oldExitTime = dt.datetime.strptime(infoBets["DT2"].iloc[e-1],"%Y.%m.%d %H:%M:%S")
        newEntryTime = dt.datetime.strptime(infoBets["DT1"].iloc[e],"%Y.%m.%d %H:%M:%S")

        extendExitMarket = (newEntryTime-oldExitTime<=dt.timedelta(0))
        #print(extendExitMarket)
        if (Journal['Asset'].iloc[e] == Journal['Asset'].iloc[e-1] 
            and extendExitMarket and np.abs(accDirectionAllInd[e-1])<2):
            #print("continue")
            continue
        else:
            thisSpread = (infoBets["A2"].iloc[e-1]-infoBets["B2"].iloc[e-1])/infoBets["B1"].iloc[e-1]
            rSpread += thisSpread
            GROI = np.sign(expectations[eInit])*(infoBets["B2"].iloc[e-1]-infoBets["B1"
                            ].iloc[eInit])/infoBets["B1"].iloc[e-1]
            #stoploss
            sl = 2*thisSpread
            thisGROI = np.max([GROI,-sl])
            rGROI += thisGROI
            
            #print(outcomes[eInit])
            rGOIxLevel[int(np.abs(outcomes[eInit])-1)] += 100*(thisGROI-thisSpread)
            
            #print(prob[eInit])
            #print(np.floor(10*prob[eInit])/10==percents)
    ...
    if Journal.shape[0]>0:     
        ...
        thisSpread = (infoBets["A2"].iloc[-1]-infoBets["B2"].iloc[-1])/infoBets["B1"].iloc[-1]
        rSpread += thisSpread
        #print((infoBets["B2"].iloc[e]-infoBets["B1"].iloc[eInit])/infoBets["B1"].iloc[e])
        GROI = np.sign(expectations[eInit])*(infoBets["B2"].iloc[-1]-infoBets["B1"
                    ].iloc[eInit])/infoBets["B1"].iloc[-1]
        # stoploss
        sl = 2*thisSpread
        thisGROI = np.max([GROI,-sl])
        rGROI += thisGROI
    ...
    Journal.loc['Total'] = ['                    ','                      ','',
               Journal['GROI'].sum(),Journal['Spread'].sum(),Journal['ROI'].sum(),'','','','']
    
    Journal.loc['Real'] = ['                    ','                      ','',
               100*rGROI,100*rSpread,100*rROI,'','','','']
    #print(Journal)
    return Journal,rGOIxLevel,rROIxPercent

def getSummary(results,size_output_layer):
    
    
    Journal= results["Journal"].drop("Real").drop("Total")
    #print(Journal["Asset"])
    # get ROI per asset
    assets = Journal["Asset"].unique()
    #print(assets)
    ROIxAsset = np.zeros((len(assets)))
    
    for i in range(len(assets)):
        thisAssIndexes = Journal["Asset"] == assets[i]
        
        ROIxAsset[i] = np.sum(Journal["ROI"].loc[thisAssIndexes].astype(float))
    #print(thisAssIndexes)
    #print(ROIxAsset)
    # get ROI per level
    #print(Journal["Bet"].iloc[:-2].astype(int).T)
    #levels = np.sort(np.unique(np.abs(Journal["Bet"][:-2].astype(int))))
    #print("levels")
    #print(levels)
    ROIxLevel = np.zeros((int((size_output_layer-1)/2)))
    for l in range(int((size_output_layer-1)/2)):
        #print(l)
        thisLevelIndexes = Journal["Bet"].iloc[:].astype(int).abs()==l+1
        #print(thisLevelIndexes)
        #thisLevelIndexes = thisLevelIndexes.append(pd.DataFrame(data=[False,False],index=["Total","Real"]))
        #print(thisLevelIndexes)
        iSupport = np.array([i for i in range(thisLevelIndexes.shape[0])])
        iSupport = iSupport[thisLevelIndexes]
        #print(iSupport)
        ROIxLevel[l] = np.sum(Journal["ROI"].iloc[iSupport].astype(float))
    #print(ROIxLevel)
    #print(ROIxLevel)
    
    # get ROI per percent level
    percents = np.array([1, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3])
    ROIxPercent = np.zeros((percents.shape[0]-1))
    for p in range(percents.shape[0]-1):
        thisPercentIndexes = ((Journal["Prob"].iloc[:].astype(float)<percents[p])&
                               (Journal["Prob"].iloc[:].astype(float)>=percents[p+1]))
        iSupport = np.array([i for i in range(thisPercentIndexes.shape[0])])
        iSupport = iSupport[thisPercentIndexes]
        ROIxPercent[p] = np.sum(Journal["ROI"].iloc[iSupport].astype(float))
    #print(ROIxPercent)
    
    results["Assets"] = assets
    results["ROIxAsset"] = ROIxAsset
    #results["Levels"] = levels
    results["ROIxLevel"] = ROIxLevel
    results["ROIxPercent"] = ROIxPercent
    
    
    
    return results

def printResults(ID,size_output_layer,init=0,saveResults=1,resultsDir="../results/",t_index=-1):
    #print(resultsDir+"TR.p")
    if os.path.exists(resultsDir+"TR.p"):
        #print("TR exists")
        TR = pickle.load( open( resultsDir+"TR.p", "rb" ))
        if ID in TR:
    ...
    if t_index == -1:
        ...
        filename = resultsDir+ID+"/"+ID+"t"+str(t_index)+".txt"
    
    columns=['J_test', 'J_train', 'Acc','RD','NZ','NZA' ,'AccDir', 'AccDirA',
                 'perNZ','perNZA','tGROI','tROI','rGROI','rROI',
                 '30RD','30NZ','30NZA','30accD','30accDA','ROI30',
                 '40RD','40NZ','40NZA','40accD','40accDA','ROI40',
                 '50RD','50NZ','50NZA','50accD','50accDA','ROI50',
                 '60RD','60NZ','60NZA','60accD','60accDA','ROI60',
                 '70RD','70NZ','70NZA','70accD','70accDA','ROI70',
                 '80RD','80NZ','80NZA','80accD','80accDA','ROI80',
                 '90RD','90NZ','90NZA','90accD','90accDA','ROI90']
    
    # insert levels in columns
    pos_levels = 14
    for l in range(int((size_output_layer-1)/2),0,-1):
        listLevel = ['l'+str(l)+'RD','l'+str(l)+'NZ','l'+str(l)+'NZA',
                     'l'+str(l)+'accD','l'+str(l)+'accDA','ROIl'+str(l)]
        columns[pos_levels:pos_levels] = listLevel
    ...
    if os.path.exists(filename)==False:
        ...
        for column in columns:
            ...
            if column not in TRdf.columns:
                # insert new columns and drop old ones
                TRdf.insert(c,column,0)        
                #TRdf = TRdf.drop("GROI",axis=1)
            
            c += 1
        #TRdf.to_csv("test.txt",sep='\t',float_format='%.3f')
    
    #a=p
        #print(TRdf)
    for epoch in range(init,lastSaved):
        thisID = ID+str(epoch)
        results = TR[thisID]
        if t_index == -1:
        ...
        print("J_test = "+str(results["J_test"])+", J_train = "+str(results["J_train"])+", Accuracy="+str(results["accuracy"]))
        #print("".format())
        print("RD = {0:d} NZ = {1:d} NZA = {2:d} AD = {3:.2f}%, ADA = {4:.2f}%".format(
                results["RD"],results["NZ"],results["NZA"],results["accDirection"],results["accDirectionAll"]))
        #print(". AccDirA = {2:.2f}%".format(results["NZA"],results["accDirectionAll"]))
        print("Percent NZ = {0:.2f}%. Percent NZA = {1:.2f}%".format(
                results["percentNonZeros"],results["percentNonZerosAll"]))
        print("tGROI = {0:.4f}%, tSpread = {1:.4f}%, tROI = {2:.4f}%".format(
                results["tGROI"]/100,results["tSpread"]/100,results["tROI"]/100))
        print("rGROI = {0:.4f}%, rSpread = {1:.4f}%, rROI = {2:.4f}%".format(
                results["rGROI"]/100,results["rSpread"]/100,results["rROI"]/100))
        
        #print(TRdf)
        TRdf.loc[epoch] = np.zeros((TRdf.shape[1]))
        TRdf.loc[epoch]["J_test"] = 100*results["J_test"]
        TRdf.loc[epoch]["J_train"] = 100*results["J_train"]
        TRdf.loc[epoch]["Acc"] = results["accuracy"]
        TRdf.loc[epoch]["RD"] = results["RD"]
        TRdf.loc[epoch]["NZ"] = results["NZ"]
        TRdf.loc[epoch]["AccDir"] = results["accDirection"]
        #TRdf.loc[epoch]["RDA"] = results["RDA"]
        TRdf.loc[epoch]["NZA"] = results["NZA"]
        TRdf.loc[epoch]["AccDirA"] = results["accDirectionAll"]
        TRdf.loc[epoch]["perNZ"] = results["percentNonZeros"]
        TRdf.loc[epoch]["perNZA"] = results["percentNonZerosAll"]
        TRdf.loc[epoch]["tGROI"] = results["tGROI"]/100
        TRdf.loc[epoch]["tROI"] = results["tROI"]/100
        TRdf.loc[epoch]["rGROI"] = results["rGROI"]/100
        TRdf.loc[epoch]["rROI"] = results["rROI"]/100
        c = 0
        #print(results["level"+str(1)])
        #print(results["level"+str(2)])
        for l in range(int((size_output_layer-1)/2)):
            #print(results["ROIxLevel"][c])
            resultsLevel = results["level"+str(l+1)]
            print("L {0:d}: RD = {1:d}. NZ = {2:d} NZA = {3:d}. AccDir = {4:.2f}% AccDirA = {5:.2f}% ROI = {6:.4f}".format(l+1,
                  resultsLevel[0],resultsLevel[1],resultsLevel[3],
                  100*resultsLevel[0]/resultsLevel[1],100*resultsLevel[2]/resultsLevel[3],results["rROIxLevel"][c]/100))
            
            TRdf.loc[epoch]["l"+str(l+1)+"RD"] = resultsLevel[0]
            TRdf.loc[epoch]["l"+str(l+1)+"NZ"] = resultsLevel[1]
            TRdf.loc[epoch]["l"+str(l+1)+"accD"] = 100*resultsLevel[0]/resultsLevel[1]
            #TRdf.loc[epoch][str(int(100*percents[per+1]))+"RDA"] = resultsPercent[2]
            TRdf.loc[epoch]["l"+str(l+1)+"NZA"] = resultsLevel[3]
            TRdf.loc[epoch]["l"+str(l+1)+"accDA"] = 100*resultsLevel[2]/resultsLevel[3]
            TRdf.loc[epoch]["ROIl"+str(l+1)] = results["rROIxLevel"][c]/100
            #print(TRdf.loc[epoch]["ROIl"+str(l)])
            c += 1
        
        
        for per in range(percents.shape[0]-1):
            resultsPercent = results["percent"+str(int(100*percents[per+1]))]
            #print(resultsPercent)
            # rD=right direction
            TRdf.loc[epoch][str(int(100*percents[per+1]))+"RD"] = resultsPercent[0]
            TRdf.loc[epoch][str(int(100*percents[per+1]))+"NZ"] = resultsPercent[1]
            TRdf.loc[epoch][str(int(100*percents[per+1]))+"accD"] = 100*resultsPercent[0]/resultsPercent[1]
            #TRdf.loc[epoch][str(int(100*percents[per+1]))+"RDA"] = resultsPercent[2]
            TRdf.loc[epoch][str(int(100*percents[per+1]))+"NZA"] = resultsPercent[3]
            TRdf.loc[epoch][str(int(100*percents[per+1]))+"accDA"] = 100*resultsPercent[2]/resultsPercent[3]
            
            TRdf.loc[epoch]["ROI"+str(int(100*percents[per+1]))] = results["rROIxPercent"][per]/100
            #print(TRdf["ROI"+str(int(100*percents[per+1]))])
            #TRdf.loc[epoch][str(int(100*percents[per+1]))+"accDA"] = rP2
            print("{0:.0f}%: RD = {1:d}. NZ = {2:d} NZA = {3:d}. AccDir = {4:.2f}% AccDirA = {5:.2f}% ROI = {6:.4f}".format(100*percents[per+1],
                  resultsPercent[0],resultsPercent[1],resultsPercent[3],
                  100*resultsPercent[0]/resultsPercent[1],100*resultsPercent[2]/resultsPercent[3],results["rROIxPercent"][per]/100))
    
    for l in range(int((size_output_layer-1)/2)):
        TRdf["l"+str(l+1)+"RD"] = TRdf["l"+str(l+1)+"RD"].astype(int).fillna(0)
        TRdf["l"+str(l+1)+"NZ"] = TRdf["l"+str(l+1)+"NZ"].astype(int).fillna(0)
        TRdf["l"+str(l+1)+"accD"] = TRdf["l"+str(l+1)+"accD"].fillna(0)
        TRdf["l"+str(l+1)+"NZA"] = TRdf["l"+str(l+1)+"NZA"].astype(int).fillna(0)
        TRdf["l"+str(l+1)+"accDA"] = TRdf["l"+str(l+1)+"accDA"].fillna(0)
        TRdf["ROIl"+str(l+1)] = TRdf["ROIl"+str(l+1)].fillna(0)
            
    for per in range(percents.shape[0]-1):
        TRdf[str(int(100*percents[per+1]))+"RD"] = TRdf[str(int(100*percents[per+1]))+"RD"].astype(int)
        TRdf[str(int(100*percents[per+1]))+"RD"] = TRdf[str(int(100*percents[per+1]))+"RD"].fillna(0)
        TRdf[str(int(100*percents[per+1]))+"NZ"] = TRdf[str(int(100*percents[per+1]))+"NZ"].astype(int)
        TRdf[str(int(100*percents[per+1]))+"NZ"] = TRdf[str(int(100*percents[per+1]))+"NZ"].fillna(0)
        #TRdf[str(int(100*percents[per+1]))+"accD"] = TRdf[str(int(100*percents[per+1]))+"accD"].astype(int)
        TRdf[str(int(100*percents[per+1]))+"accD"] = TRdf[str(int(100*percents[per+1]))+"accD"].fillna(0)
        #TRdf[str(int(100*percents[per+1]))+"RDA"] = TRdf[str(int(100*percents[per+1]))+"RDA"].astype(int)
        #TRdf[str(int(100*percents[per+1]))+"RDA"] = TRdf[str(int(100*percents[per+1]))+"RDA"].fillna(0)
        TRdf[str(int(100*percents[per+1]))+"NZA"] = TRdf[str(int(100*percents[per+1]))+"NZA"].astype(int)
        TRdf[str(int(100*percents[per+1]))+"NZA"] = TRdf[str(int(100*percents[per+1]))+"NZA"].fillna(0)
        #TRdf[str(int(100*percents[per+1]))+"accDA"] = TRdf[str(int(100*percents[per+1]))+"accDA"].astype(int)
        TRdf[str(int(100*percents[per+1]))+"accDA"] = TRdf[str(int(100*percents[per+1]))+"accDA"].fillna(0)
        TRdf["ROI"+str(int(100*percents[per+1]))] = TRdf["ROI"+str(int(100*percents[per+1]))].fillna(0)
    
    TRdf["RD"] = TRdf["RD"].astype(int)
    TRdf["NZ"] = TRdf["NZ"].astype(int)
    #TRdf["RDA"] = TRdf["RDA"].astype(int)
    TRdf["NZA"] = TRdf["NZA"].astype(int)
    if saveResults:
        TRdf.sort_index().to_csv(filename,sep='\t',float_format='%.3f')
...
def save_results_v10(TRdf, t_index, thr_mc, epoch, lastTrained, save_results=0, resultsDir="",ID=""):
    ...
    if save_results:
        ...
        if epoch==lastTrained:#TRdf.shape[0]==1:
            for th in [.5,.6,.7,.8,.9]:
                TRdf[str(th)+"RD"] = TRdf[str(th)+"RD"].astype(int).fillna(0)
                TRdf[str(th)+"NZ"] = TRdf[str(th)+"NZ"].astype(int).fillna(0)
                TRdf[str(th)+"NZA"] = TRdf[str(th)+"NZA"].astype(int).fillna(0)
                TRdf[str(th)+"AD"] = TRdf[str(th)+"AD"].fillna(0)
                TRdf[str(th)+"ADA"] = TRdf[str(th)+"ADA"].fillna(0)
                TRdf[str(th)+"pNZ"] = TRdf[str(th)+"pNZ"].fillna(0)
                TRdf[str(th)+"pNZA"] = TRdf[str(th)+"pNZA"].fillna(0)
                TRdf[str(th)+"GROI"] = TRdf[str(th)+"GROI"].fillna(0)
                TRdf[str(th)+"rROI"] = TRdf[str(th)+"rROI"].fillna(0)
...
def print_results_v10(TRdf, results, epoch, thr_md, thr_mc, t_index, save_results = 0):
    
    print("Epoch = "+str(epoch)+". Time index = "+str(t_index)+". Threshold MC = "+str(thr_mc)+". Threshold MD = "+str(thr_md))
    if thr_md==.5 and thr_mc==.5:
        print("J_test = "+str(results["J_test"])+", J_train = "+str(results["J_train"])+", Accuracy="+str(results["Acc"]))
    print("RD = {0:d} NZ = {1:d} NZA = {2:d} pNZ = {5:.3f}% pNZA = {6:.3f}% AD = {3:.2f}% ADA = {4:.2f}%.".format(
            results["RD"],results["NZ"],results["NZA"],results["AccDir"],results["AccDirA"],results["perNZ"],results["perNZA"]))
    #print(". AccDirA = {2:.2f}%".format(results["NZA"],results["accDirectionAll"]))
    print("Sharpe = {1:.3f} rRl1 = {2:.4f}% rRl2 = {3:.4f}% rGROI = {4:.4f}% rROI = {0:.4f}% tGROI = {5:.4f}% tROI = {6:.4f}%".format(
            results["rROI"],results["sharpe"],results["rROIxLevel"][0],results["rROIxLevel"][1],
            results["rGROI"],results["tGROI"],results["tROI"]))
    ...
    if save_results:
        ...
        TRdf.loc[epoch]["J_train"] = 100*results["J_train"]
        TRdf.loc[epoch]["AccMC"] = results["Acc"]
        TRdf[str(thr_md)+"RD"].loc[epoch] = results["RD"]
        TRdf[str(thr_md)+"NZ"].loc[epoch] = results["NZ"]
        TRdf[str(thr_md)+"AD"].loc[epoch] = results["AccDir"]
        TRdf[str(thr_md)+"NZA"].loc[epoch] = results["NZA"]
        TRdf[str(thr_md)+"ADA"].loc[epoch] = results["AccDirA"]
        TRdf[str(thr_md)+"pNZ"].loc[epoch] = results["perNZ"]
        TRdf[str(thr_md)+"pNZA"].loc[epoch] = results["perNZA"]
        TRdf[str(thr_md)+"GROI"].loc[epoch] = results["rGROI"]/100
        TRdf[str(thr_md)+"rROI"].loc[epoch] = results["rROI"]/100
...
def print_and_save_results_v20(results, epoch, t_index, thr_mc, thr_md, save_results=0, resultsDir="",ID=""):
    """
    Version 2.0 of print results function. Compatible with RNN networks with yc values
    """
    #print("Epoch "+str(epoch)+" "+"t_index="+str(t_index)+":")
    print("Epoch = "+str(epoch)+". Time index = "+str(t_index)+". Threshold MC = "+str(thr_mc)+". Threshold MD = "+str(thr_md))
    print("J_test = "+str(results["J_test"])+", J_train = "+str(results["J_train"])+", Accuracy="+str(results["Acc"]))
    #print("".format())
    print("RD = {0:d} NZ = {1:d} NZA = {2:d} AD = {3:.2f}%, ADA = {4:.2f}%".format(
            results["RD"],results["NZ"],results["NZA"],results["AccDir"],results["AccDirA"]))
    #print(". AccDirA = {2:.2f}%".format(results["NZA"],results["accDirectionAll"]))
    print("Percent NZ = {0:.3f}%. Percent NZA = {1:.3f}%. rROI = {2:.4f}. Sharpe = {3:.2f}".format(
            results["perNZ"],results["perNZA"],results["rROI"]/100,results["sharpe"]))
    ...
    if save_results:
        ...
        columns=['thr_mc','thr_md','J_test', 'J_train', 'AccMC','pNZ','pNZA','RD','NZ','NZA','AD','ADA','rROI','rGROI','tROI','tGROI']
        ...
        TRdf[str(thr_md)+"pNZ"].loc[epoch] = results["perNZ"]
        TRdf[str(thr_md)+"pNZA"].loc[epoch] = results["perNZA"]
        TRdf[str(thr_md)+"GROI"].loc[epoch] = results["rGROI"]
        TRdf[str(thr_md)+"rROI"].loc[epoch] = results["rROI"]
        TRdf[str(thr_md)+"GROI"].loc[epoch] = results["tGROI"]
        TRdf[str(thr_md)+"rROI"].loc[epoch] = results["tROI"]
        
        # format table
        #for th in [.5,.6,.7,.8,.9]:
        TRdf[str(thr_md)+"RD"] = TRdf[str(thr_md)+"RD"].astype(int).fillna(0)
        TRdf[str(thr_md)+"NZ"] = TRdf[str(thr_md)+"NZ"].astype(int).fillna(0)
        TRdf[str(thr_md)+"NZA"] = TRdf[str(thr_md)+"NZA"].astype(int).fillna(0)
        TRdf[str(thr_md)+"AD"] = TRdf[str(thr_md)+"AD"].fillna(0)
        TRdf[str(thr_md)+"ADA"] = TRdf[str(thr_md)+"ADA"].fillna(0)
        TRdf[str(thr_md)+"pNZ"] = TRdf[str(thr_md)+"pNZ"].fillna(0)
        TRdf[str(thr_md)+"pNZA"] = TRdf[str(thr_md)+"pNZA"].fillna(0)
        TRdf[str(thr_md)+"GROI"] = TRdf[str(thr_md)+"GROI"].fillna(0)
        TRdf[str(thr_md)+"rROI"] = TRdf[str(thr_md)+"rROI"].fillna(0)
        
        #print("TR saved")
        #print(TRdf)
        TRdf.sort_index().to_csv(filename,sep='\t',float_format='%.3f')
        
        # save best performance apart
        #TRdf.
        
    return None

def print_and_save_results_v10(results, epoch, t_index, thr_mc, thr_md, save_results=0, resultsDir="",ID=""):
    """
    Version 1.0 of print results function. Compatible with RNN networks with yc values
    """
    #print("Epoch "+str(epoch)+" "+"t_index="+str(t_index)+":")
    print("Epoch = "+str(epoch)+". Time index = "+str(t_index)+". Threshold MC = "+str(thr_mc)+". Threshold MD = "+str(thr_md))
    print("J_test = "+str(results["J_test"])+", J_train = "+str(results["J_train"])+", Accuracy="+str(results["Acc"]))
